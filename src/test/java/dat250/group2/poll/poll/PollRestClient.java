...
class PollRestClient {
    ...
    void fullScenario() {
        ...
        assertThat(createdUser2.getUserId()).isEqualTo("user2");
        ...
        assertThat(Arrays.stream(users).map(User::getUserId)).contains("Dummy", "user2");
        ...
        Vote latestfromuser2 = Arrays.stream(votes)
                .filter(v -> v.getUser() != null && "user2".equals(v.getUser().getUserId()))
